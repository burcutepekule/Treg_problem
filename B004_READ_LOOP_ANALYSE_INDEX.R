...
if(dim(all_params_merged_keep_9)[1]>0){
...
if(dim(all_params_merged_keep_10)[1]>0){
...
if(dim(all_params_merged_keep_11)[1]>0){
...
if(dim(lda_pred$x)[2]>1){
  ...
  loadings$importance = abs(loadings$LD1)
  top_features = loadings %>% filter(importance > 0.25*max(loadings$importance))
  ...
  p_lda_sterile_1 = ggplot(lda_scores, aes(x = LD1, fill = class_label)) +
    geom_density(alpha = 0.4) +
    geom_segment(data = top_features,
                 aes(x = 0, xend = LD1, y = 0, yend = -0.01),  # yend sets vertical offset below density curve
                 arrow = arrow(length = unit(0.25, "cm")),
                 color = "black", linewidth = 1.1,
                 inherit.aes = FALSE) +
    geom_text(data = top_features,
              aes(x = LD1, y = -0.02, label = feature),
              angle = 45, hjust = 0, vjust = 1,
              size = 3.5, fontface = "bold",
              position = position_nudge(y = -0.02),
              inherit.aes = FALSE)+
    scale_fill_manual(values = custom_colors) +
    labs(title = "LDA projection (1D) with top features, sterile injury",
         x = "LD1", fill = "Treg effect class") +
    theme_minimal(base_size = 14) +
    xlim(NA, 5) +
    theme(plot.margin = margin(10, 10, 30, 10))  # more bottom margin for text
}
